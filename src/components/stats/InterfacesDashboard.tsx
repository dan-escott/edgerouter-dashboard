...
export const InterfacesDashboard = (props: { statsService: StatsService }) => {
  ...
  useEffect(() => {
    if (tiles.length === 0) {
      ...
      .then((json: Configuration) => {
        setTiles(Object.keys(json.GET.interfaces.ethernet).map(id => {
          return {
            id: id,
  ...
  return <Dashboard>
      {
        tiles.map(tile => (
          <InterfaceTile key={tile.id} reverse={tile.id !== 'eth0'} id={tile.id} name={tile.name} statsService={props.statsService} />
        ))
      }
  </Dashboard>
}
